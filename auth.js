(() => {
  ...
  async function login(password){
    const cred = JSON.parse(localStorage.getItem(KEY)||'null');
    if(!cred) throw new Error('Not set yet');
    const salt = fromB64(cred.salt);
    const want = cred.hash;
    const got = toB64(await hashPassword(password, salt));
    if(got === want){ sessionStorage.setItem(ROLE,'1'); return true; }
    return false;
  }
